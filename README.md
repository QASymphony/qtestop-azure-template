...
## How
During qTest application start up, the qtestctl is executed by the system via command `./qtestctl start`, we will inject **qtest-azure-bootstapper** for it to obtain the public IP address of the VM (by executing the command ```dig +short myip.opendns.com @resolver1.opendns.com``` (Linux) or sending HTTP request to outside web services (Windows), e.g. http://httpbin.org, https://www.ipify.org/). When succesful, **qtest-azure-bootstapper** update *qtest.config* file with the new public IP address and the application URLs in qTest database.
